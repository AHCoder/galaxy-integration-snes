...
CONFIG_LOC = r"%LOCALAPPDATA%\GOG.com\Galaxy\Configuration\plugins\snes\config.ini"

class Config:
    def __init__(self):
        self.cfg = configparser.ConfigParser(allow_no_value=True)
        self.cfg.set("DEFAULT", "; Make sure to use / instead of \ in file paths.")
        self.cfg["DEFAULT"]["roms_path"] = "C:/Games/SNES"
        self.cfg["DEFAULT"]["emu_path"] = "C:/Program Files (x86)/bsnes/bsnes.exe"
        ...
        self.cfg.set("Paths", textwrap.dedent(
                """\
                ; Set your roms folder, path to your bsnes.exe here\
                """
            )
        )
